...
public class Work_Done extends AppCompatActivity {
    private List<BusinessList02> businessList=new ArrayList<>();
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        tv1.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                Intent intent=new Intent(Work_Done.this,Work_Done.class);
                intent.setFlags(Intent.FLAG_ACTIVITY_CLEAR_TOP);
                startActivity(intent);
            }
        });
        sendRequestWithHttpURLConnection("Finished");
        handler = new Handler() {
            @Override
                ...
                switch (msg.what) {
                    case MESSAGE_OK:
                        ...
                        try {
                            ...
                            LinearLayoutManager layoutManager=new LinearLayoutManager(Work_Done.this);
                            recyclerView.setLayoutManager(layoutManager);
                            BusinessListAdapter02 adapter=new BusinessListAdapter02(businessList);
    ...
    private void initBusinessLists(int count,JSONObject json){
        for (int i = 1; i <= count; i++){
            ...
            try {
                ...
                segment = subJson.getString("PREVIOUSACTIVITYOS");
                time = subJson.getString("JobStartedOS");
                binderDocIDOS = subJson.getString("BinderDocIDOS");
            }catch (Exception e){
                e.printStackTrace();
            }
            BusinessList02 b = new BusinessList02(type,name,segment,time,binderDocIDOS);
            businessList.add(b);
        }
    }
        ...
        new Thread(new Runnable() {
            ...
            public void run() {
                try {
                    ...
                    e.getMessage();
                    Looper.prepare();
                    Toast.makeText(Work_Done.this,"请检查网络设置",Toast.LENGTH_SHORT).show();
                    Looper.loop();
                }
            }
        }).start();
